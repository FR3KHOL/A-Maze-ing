"""Core module for maze generation, solving, and rendering."""
import os
import time
import random
from collections import deque
from mazegen.config_handler import Settings
from mazegen.components import Node, Palette


class MazeCore:
    """Main generator and solver for the labyrinth."""

    def __init__(self, cfg_path: str, use_anim: bool = False) -> None:
        """Initialize the maze core.

        Args:
            cfg_path (str): Path to the settings file.
            use_anim (bool): Whether to show generation animation.
        """
        self.cfg = Settings(cfg_path)
        self.anim = use_anim
        self.grid: list = []
        if self.cfg.rng_seed is not None:
            self.seed_val = self.cfg.rng_seed
        else:
            self.seed_val = random.randint(0, 999999)
        random.seed(self.seed_val)
        self.optimal_path = ""
        self._setup_grid()

    def _setup_grid(self) -> None:
        """Initialize the grid array and apply the mandatory pattern."""
        self.grid = [
            [Node(15, False) for _ in range(self.cfg.cols)]
    ...
    def _knock_down(self, n: Node, d: str) -> None:
        """Remove a wall from a node.

        Args:
            n (Node): The node to modify.
            d (str): Direction of the wall to remove (N, E, S, W).
        """
        n.borders &= ~{'N': 1, 'E': 2, 'S': 4, 'W': 8}[d]

    def build_maze(self) -> None:
        """Generate the maze pathways using Iterative DFS."""
        sx = self.seed_val % self.cfg.cols
        sy = self.seed_val % self.cfg.rows
    ...
    def _add_loops(self) -> None:
        """Randomly break extra walls to create an imperfect maze."""
        dirs = [
            (-1, 0, 'W', 'E'), (1, 0, 'E', 'W'),
            (0, -1, 'N', 'S'), (0, 1, 'S', 'N')
        ]
            ...
            for x in range(self.cfg.cols):
                if random.random() < 0.1 and not self.grid[y][x].is_locked:
                    ...
                    for dx, dy, d1, d2 in dirs:
                        nx, ny = x + dx, y + dy
                        if 0 <= nx < self.cfg.cols and 0 <= ny < self.cfg.rows:
                            if not self.grid[ny][nx].is_locked:
    ...
    def _find_shortest_path(self) -> None:
        """Solve the maze using BFS to find the optimal route."""
        ...
        while queue:
            ...
            moves = []
            if not (b & 1):
                moves.append((cx, cy - 1, 'N'))
            if not (b & 2):
                moves.append((cx + 1, cy, 'E'))
            if not (b & 4):
                moves.append((cx, cy + 1, 'S'))
            if not (b & 8):
                moves.append((cx - 1, cy, 'W'))
    ...
    def export_to_file(self) -> None:
        """Write the generated maze data and solution to the output file."""
    ...
    def draw(
        ...
        """Render the maze layout to the terminal.

        Args:
            clr (str): ANSI color code for the maze walls.
            char_wall (str): Characters representing walls.
            show_sol (bool): Whether to display the shortest path.
            sol_char (str): Characters representing the path.
            start_char (str): Characters representing the start point.
            end_char (str): Characters representing the end point.
        """
        os.system('clear')
        path_coords = set()
        ...
        for y, row in enumerate(self.grid):
            ...
            for x, node in enumerate(row):
                is_start = (
                    x == self.cfg.start_pt['x'] and y == self.cfg.start_pt['y']
                )
                is_end = (
                    x == self.cfg.end_pt['x'] and y == self.cfg.end_pt['y']
                )
                ...
                top_line += (
                    (char_wall * 2) if node.borders & 1 else f"{char_wall}  "
                )
        ...
        for node in self.grid[-1]:
            bottom_line += (
                (char_wall * 2) if node.borders & 4 else f"{char_wall}  "
            )
